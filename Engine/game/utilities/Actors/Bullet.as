...
package utilities.Actors{
	...
	public class Bullet extends Actor{
		...
		private function doStuffToBulletOverTime(){
		...
		private function applyVector(){
			...
			var totalmove = Math.sqrt(xVelocity * xVelocity + yVelocity * yVelocity);
		...
		public function resumeBulletTime(){
		...
		private function checkForLifespanExpired(){
		...
		public function setInitialLocationAndVector(){
		...
		private function change_vector_based_on_bullet_properties(){
			if(utilities.Actors.Stats.WeaponStats.getBlobular() > 0){
		...
		private function applyBulletBehaviors(){
			...
			if(utilities.Actors.Stats.WeaponStats.getHeatSeeking() >0){
				...
				if(get_hasTargetFlag()==true && getTarget() != null){
					var newVels = utilities.Actors.Behaviors.Behavior_HeatSeeking.updateVelocities();
					xVelocity = newVels.x;
					yVelocity = newVels.y;
					//trace(this.x);
				}
