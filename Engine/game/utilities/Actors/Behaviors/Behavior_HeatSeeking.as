...
package utilities.Actors.Behaviors{
	...
	public class Behavior_HeatSeeking extends MovieClip{
		...
		public static function heatSeeking(bullet,deltaX,deltaY,xVelocity,yVelocity){
		...
		private static function findNewTarget(bullet){
			var prevDistance = int.MAX_VALUE;
			for (var i:int = 0; i<EnemyManager.enemies.length;i++){
				var dist = MathFormulas.distanceFormula(new Point(EnemyManager.enemies[i].x,EnemyManager.enemies[i].y),new Point(bullet.x,bullet.y));
		...
		private static function seekTarget(bullet,deltaX,deltaY,xVelocity,yVelocity){
			var newTarget = bullet.getTarget();
			//trace("seeking");
