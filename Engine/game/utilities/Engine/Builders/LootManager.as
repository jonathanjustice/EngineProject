package utilities.Engine.Builders{
	...
	public class LootManager extends utilities.Engine.DefaultManager{
		...
		private static function updateLootDrops(){
		...
		private static function updateTreasureChests(){
			for each(var chest:TreasureChest in treasureChests){
				chest.updateLoop();//controls opening progress & states
				var avatar = Game.avatarManager.getAvatar();
				if(chest.hitTestObject(avatar)){
		...
		public static function check_For_Collision_With_Avatar(lootDrop){
		...
		public function createNewLootDrop(spawnX,spawnY,type,amt){
			var amount = amt;
			var loot;
		...
		public static function deleteTreasureChest(chest){
		...
		public function createNewTreasureChest(locationX,locationY){
